...
int64_t FileHandle::Read(QueryContext context, void *buffer, idx_t nr_bytes) {
	const bool track = track_io && context.GetClientContext() != nullptr;
	const auto start = track ? steady_clock::now() : time_point<steady_clock>();
	...
	if (track) {
		const auto elapsed_us =
		    UnsafeNumericCast<idx_t>(duration_cast<microseconds>(steady_clock::now() - start).count());
		QueryProfiler::Get(*context.GetClientContext()).TrackBytesRead(UnsafeNumericCast<idx_t>(bytes_read), elapsed_us);
	}
