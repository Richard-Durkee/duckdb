...
public:
	...
	//! Track bytes read and the time spent reading (always tracked, even when profiling disabled).
	DUCKDB_API void TrackBytesRead(idx_t amount, idx_t elapsed_us);
	//! Track bytes written and the time spent writing (always tracked, even when profiling disabled).
