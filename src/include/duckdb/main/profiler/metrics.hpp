...
// Io metrics
struct MetricIONetworkAggregateBandwidthBytesPerS {
	using METRIC_TYPE = double;
	static constexpr const char *Name = "io.network_aggregate_bandwidth_bytes_per_s";
	static constexpr const char *Description = "Aggregate delivered read bandwidth for remote (network) file systems: total remote bytes over the query's wall-clock time, capturing parallel range requests; zero when no remote reads occurred";
...
struct MetricIONetworkBytesRead {
	using METRIC_TYPE = uint64_t;
	static constexpr const char *Name = "io.network_bytes_read";
	static constexpr const char *Description = "Total bytes read from remote (network) file systems; zero when no remote reads occurred";
	static constexpr const char *Unit = "bytes";
	static constexpr const char *TypeStr = "uint64";
};
